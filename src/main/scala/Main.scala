...
object Main extends App {
  ...
  args match {
    ...
    case Array() => repl()
    case x => println(s"Unexpected number of arguments (${x.length}).")
  }

  def run(path: String): Unit = {
    val prog = Parser.parseFile(path)

    // Build an initial type environment from annotations and bindings.
    implicit val tenv = InferenceEngine.buildEnv(prog)

    // Run type inference on each statement.
    prog.infer
        ...
        if (input.startsWith(":")) {
          parseCommand(input) match {
            ...
            case CType(expr) => expr.infer match {
              case (_, typ, _) => println(s"${expr.pretty} :: ${typ.pretty}")
            }
          }
        } else {
